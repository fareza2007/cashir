...
// Vercel Serverless Function — Buat transaksi Midtrans Snap
...
module.exports = async (req, res) => {
  ...
  const payload = JSON.stringify({
    ...
    callbacks: {
      finish: `https://cashir.pied.vercel.app`,
    },
  });

  // Tentukan URL Midtrans (production vs sandbox)
  ...
  const snapToken = await new Promise((resolve, reject) => {
    const httpReq = https.request(options, (httpRes) => {
      ...
      httpRes.on('end', () => {
        try {
          ...
          else reject(new Error(parsed.error_messages?.join(', ') || 'Midtrans error'));
        } catch (e) {
          reject(e);
        }
      });
